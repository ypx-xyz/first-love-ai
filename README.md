...
下图为系统**全链路技术架构**：从多源语料采集、标准化、蒸馏沉淀到知识 Wiki，再由 LLM 组装上下文生成回复，最终经 Flask 服务层对外输出，形成「生成 → 审计 → 约束更新」的闭环。

![系统全链路技术架构](docs/assets/architecture.png)

> 图中两条虚线为反馈回路：左侧「**新鲜语料直灌**」保证推理层拿到现生内容，右侧「**防漂移**」把生成结果回锚稳定记忆；蒸馏层与知识层之间还有「**落库前交叉核对**」的校准点。
